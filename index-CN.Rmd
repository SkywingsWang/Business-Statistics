...
该项目旨在通过对未能达到"基本满意"要求的餐饮单位是否对地方当局的干预措施做出了有效反应，地方当局成功执行的执法行动与专业执法人员数量的关系、和执法人员与餐饮单位数量的比例之间的关系进行分析，找出影响餐饮单位对干预行动反应成功率的因素，为地方当局的决策提供依据，以提高整体食品安全水平。

**该项目的报告分为两部分**。第一部分针对专业人士，详细展现了我们的数据分析过程和结果。第二部分则针对政策制定者，概述了我们的主要发现和建议。我们希望通过这种方式，使得无论读者的背景如何，都能够理解和利用我们的研究成果。
...
food_data$FTE <- cut(food_data$`ProfessionalFullTimeEquivalentPosts-occupied *`,breaks = c(0,2,4,6,23))
...
ggplot(food_data,aes(x=FTE,y=`Total%ofInterventionsachieved-premisesratedD`))+geom_boxplot()+
  ...
  labs(x="执法人员数量",y="干预成功率 (%)",subtitle = "箱型图")+ylim(0,100),
...
# 由于模型的拟合度过低，通过要求 FTE <= 15 来消除一些可能的异常值，再次对 D 级干预的百分比进行回归，查看是否能带来影响
...
在建立回归模型之前，我们使用散点图来直观显示相关变量之间的关系。无论是整体衡量，还是按 A、B、C、D 和 E 类分别衡量，“干预成功率”与“执法人员数量”本身之间似乎都没有明显的关系，**除 D 类机构外，蓝色拟合线都是近似水平的**。为了更好地观察这些过于集中的数据点，同时减轻异常值的影响，我们将连续变量“执法人员数量”切分为四个区间，并绘制了额外的箱形图。
...
就被评为 A、B、C、D 和 E 级的单位而言，**执法人员相对于 1000 家餐饮单位的比例对风险较低的机构（被评为 C、D 和 E 级）的干预效果似乎更强**。如图所示，蓝色拟合线从 A 到 E 越来越陡。
...
**对于被认为风险较低的 C 级、D 级和 E 级单位，增加专业人员的数量会显著提高干预成功率**。这可能是由于这些较低风险的单位在资源有限的情况下没有成为政府干预的首要目标。因此，对于这类单位，在资源充足的情况下可以尝试通过提高执法人员相对于餐饮单位的比例来提高干预成功率。
